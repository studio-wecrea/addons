...
{
  private const COUNTRY_LIST = array(
  ...
  public static function all($name = null, $lang = 'en')
  {
    $filtered = empty($name) ? self::COUNTRY_LIST : array_filter(self::COUNTRY_LIST, function ($row) use ($name, $lang) {
      return preg_match("/$name/i", $row[$lang]);
    });

    return array_map(function ($row) use ($lang) {
      return $row[$lang];
    }, $filtered);
  }
  ...
  {
    if (!isset(self::COUNTRY_LIST[$code])) {
      return null;
    }

    if (!isset(self::COUNTRY_LIST[$code][$lang])) {
      return null;
    }

    return self::COUNTRY_LIST[$code][$lang];
  }
}